...
export default function ProductsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!storeSlug) return
    if (!form.image_url) { setError('Subí una imagen para la prenda.'); return }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 h-[40px] rounded-[10px] font-semibold transition-colors"
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center rounded-[16px]" style={{ backgroundColor: '#FFFFFF' }}>
          ...
          <p className="mt-3 font-medium" style={{ fontSize: '15px', color: '#6E6E73' }}>
            No cargaste ninguna prenda todavía.
          </p>
          <button
            onClick={() => setShowForm(true)}
          ...
          {products.map((p) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="capitalize" style={{ fontSize: '13px', color: '#6E6E73' }}>
                  {p.category}
                  {p.price ? ` · $${p.price.toLocaleString('es-AR')}` : p.price_range ? ` · ${p.price_range}` : ''}
                </p>
              </div>
              <button
                onClick={() => handleDelete(p.id)}
                className="flex-none p-2 rounded-[8px] transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-end" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Image upload */}
              <div>
                <p className="text-[13px] font-medium mb-2" style={{ color: '#1D1D1F' }}>Foto de la prenda *</p>
                {form.image_url ? (
                  <div className="relative w-full rounded-[12px] overflow-hidden" style={{ aspectRatio: '3/4', maxWidth: '140px' }}>
                  ...
                  <label
                    ...
                    style={{ height: '120px', borderRadius: '12px', backgroundColor: '#F5F5F7', border: '2px dashed #D2D2D7' }}
                  >
                    {uploading ? (
                      ...
                      <>
                        ...
                        <span className="mt-1" style={{ fontSize: '13px', color: '#AEAEB2' }}>Elegir foto</span>
                      </>
                    )}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Campera bomber negra"
                  required
                ...
                <div className="flex gap-2">
                  {PRICES.map((pr) => (
                    <button
                      ...
                      className="flex-1 py-2 rounded-[10px] text-[13px] font-medium capitalize transition-colors"
                      style={{
                        backgroundColor: form.price_range === pr ? '#0071E3' : '#F5F5F7',
                        color: form.price_range === pr ? '#FFFFFF' : '#1D1D1F',
                      }}
                    >
                      {pr}
                    </button>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map((c) => (
                    <button
                      ...
                      style={{
                        backgroundColor: form.category === c ? '#0071E3' : '#F5F5F7',
                        color: form.category === c ? '#FFFFFF' : '#1D1D1F',
                      }}
                    >
                      {c}
                    </button>
                ...
                <div className="flex gap-2">
                  {GENDERS.map((g) => (
                    <button
                      ...
                      className="flex-1 py-2 rounded-[10px] text-[13px] font-medium capitalize transition-colors"
                      style={{
                        backgroundColor: form.gender === g ? '#0071E3' : '#F5F5F7',
                        color: form.gender === g ? '#FFFFFF' : '#1D1D1F',
                      }}
                    >
                      {g}
                    </button>
              ...
              {/* Style tags */}
              <div>
                <p className="text-[13px] font-medium mb-1.5" style={{ color: '#1D1D1F' }}>Estilo (varios)</p>
                <div className="flex flex-wrap gap-2">
                  {STYLES.map((s) => (
                    <button
                      key={s}
                      ...
                      onClick={() =>
                        setForm((f) => ({
                          ...f,
                          style_tags: f.style_tags.includes(s)
                            ? f.style_tags.filter((x) => x !== s)
                            : [...f.style_tags, s],
                        }))
                      }
                      className="px-3 py-1.5 rounded-full text-[13px] font-medium capitalize transition-colors"
                      style={{
                        backgroundColor: form.style_tags.includes(s) ? '#0071E3' : '#F5F5F7',
                        color: form.style_tags.includes(s) ? '#FFFFFF' : '#1D1D1F',
                      }}
                    >
                      {s}
                    </button>
              ...
              <div className="flex items-center justify-between py-1">
                <div>
                  <p className="font-medium" style={{ fontSize: '15px', color: '#1D1D1F' }}>Destacar prenda</p>
                  <p style={{ fontSize: '13px', color: '#6E6E73' }}>Aparece primero en tu perfil (máx. 6)</p>
                </div>
                <button
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, is_featured: !f.is_featured }))}
                  className="relative w-12 h-7 rounded-full transition-colors"
                  style={{ backgroundColor: form.is_featured ? '#0071E3' : '#D2D2D7' }}
                >
                  <div
                    className="absolute top-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform"
                    style={{ transform: form.is_featured ? 'translateX(22px)' : 'translateX(2px)' }}
                  />
                </button>
              </div>
